...
Projectile::Projectile(sf::Vector2f origin, float angleRad, float damage,
    ...
    : m_pos(origin), m_angleRad(angleRad), m_damage(damage)
    , m_texFireball(texFireball)
{
    m_velocity = {
    ...
    if (m_texFireball) {
        ...
        float deg = angleRad * 180.f / 3.14159265f;
...
void Projectile::draw(sf::RenderWindow& window) const {
    ...
    if (m_state == ProjState::Flying) {
        ...
        inner.setOrigin(ri, ri);
        inner.setPosition(m_pos);
        window.draw(inner);
    }
}